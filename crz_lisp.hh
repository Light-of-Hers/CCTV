...
template<typename RecEnv, typename Var,
    typename Params, typename Body, typename Env, typename SavedEnv, bool MoreEnv>
struct lookup_var_<RecEnv, Var, recrs_env<Var, closure<Params, Body, Env>, SavedEnv, MoreEnv>> {
    using type = closure<Params, Body, RecEnv>;
};
template<typename RecEnv, typename Var,
...
using foldl = eval<
    _<lambda, _<V(fun), V(init), V(lst)>,
        _<letrec,
            _<
                _<V(iter), _<lambda, _<V(acc), V(lst)>,
                    _<iff, _<is_null, lst>,
                        acc,
